...
function Invoice({services}) {
  ...
  const submitInvoice = (e) => {
    e.preventDefault();
    setLoading(true);
    Inertia.post('/service-invoice', {items,patient,address,foc,discount,payment},{
        preserveScroll: true,
        onSuccess: () => {
            setLoading(false);
            console.log(
            ...
            setPatient('');
            setAddress('');
            window.location.href = "/admin/service-orders";
        },
        onError: (error) => {
  ...
  return (
    <div className='w-3/4 mx-auto bg-gray-200 p-4 mt-3 text-whit max-h-full'>
      ...
      <div className='w-full text-center gap-2' >
          <a href='/admin/service-orders' className='px-4  py-2 bg-sky-400 hover:bg-sky-500 mr-3 rounded'>Back</a>
